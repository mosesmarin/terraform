...
# Configuration:  with final snapshot, encryption, 2 host in primary region, 1 host in secondary region, 1 security group
# 
#  
...
variable "pri_kms_key_arn" {
  type = string
}

variable "sec_kms_key_arn" {
  type = string
}
...
resource "aws_rds_cluster" "primary" {
  ...
  storage_encrypted               = true
  kms_key_id                      = var.pri_kms_key_arn
...
resource "aws_rds_cluster" "secondary" {
  ...
  storage_encrypted               = true
  kms_key_id                      = var.sec_kms_key_arn
